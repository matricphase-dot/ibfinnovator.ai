...
async function handleUser(data: any) {
  ...
  const primary =
    verified.find((e: any) => e.id === data.primary_email_address_id) ||
    verified[0];
  if (!primary) throw new Error("No verified email on Clerk user");
  const email = primary.email_address.trim().toLowerCase();
  const name =
    [data.first_name, data.last_name].filter(Boolean).join(" ") || "IBF Member";
  const role = ["FOUNDER", "STUDENT"].includes(data.public_metadata?.role)
  ...
  if ((matches || []).length > 1)
    throw new Error(`CONFLICT: multiple profiles use ${email}`);
  let profileId: string;
  if (matches?.[0]) {
    if (matches[0].clerk_user_id && matches[0].clerk_user_id !== data.id)
      throw new Error(
        `CONFLICT: profile already linked to ${matches[0].clerk_user_id}`,
      );
    profileId = matches[0].id;
    const { error } = await supabaseAdmin
      .from("profiles")
      .update({ clerk_user_id: data.id, name, role })
      .eq("id", profileId);
    if (error) throw error;
  } else {
    ...
    const { data: created, error: createError } =
      await supabaseAdmin.auth.admin.createUser({
        email,
        email_confirm: true,
        user_metadata: { source: "clerk", clerk_user_id: data.id, name, role },
      });
    if (createError) {
      if (!/already|registered|exists/i.test(createError.message))
        throw createError;
      shadowId = await findShadow(email);
    } else shadowId = created.user?.id || null;
    if (!shadowId)
      throw new Error("Could not create or locate shadow Supabase user");
    profileId = shadowId;
    const { error } = await supabaseAdmin
      .from("profiles")
      .upsert(
        { id: profileId, clerk_user_id: data.id, email, name, role },
        { onConflict: "id" },
      );
    if (error) throw error;
  }
...
export async function POST(req: Request) {
  ...
  if (claimError) {
    if (claimError.code !== "23505")
      return new Response(`Claim error: ${claimError.message}`, {
        status: 500,
      });
    const { data: existing, error: readError } = await supabaseAdmin
      ...
      .single();
    if (readError)
      return new Response(`Claim lookup error: ${readError.message}`, {
        status: 500,
      });
    ...
    const { data: reclaimed, error: reclaimError } = await supabaseAdmin
      ...
      .maybeSingle();
    if (reclaimError)
      return new Response(`Reclaim error: ${reclaimError.message}`, {
        status: 500,
      });
    if (!reclaimed) return new Response("ok", { status: 200 });
    claim = reclaimed;
  }
    ...
    if (evt.type === "user.deleted") {
      ...
      if (p) {
        const { error } = await supabaseAdmin
          ...
          .update({
            name: "Deleted user",
            email: `deleted+${p.id}@deleted.invalid`,
            clerk_user_id: null,
            avatar_url: null,
            bio: null,
          })
